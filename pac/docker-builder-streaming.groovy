    ...
    stages {
            ...
            steps {
                dir ("${WORKSPACE}/streaming_server/") {
                    sh '''
                         ...
                         docker build . -t 709233559969.dkr.ecr.eu-west-3.amazonaws.com/computer_vision/computer_vision_nginx:${CV_VER}
                       '''
                }
             }
          }
        stage ('push computer vision NGINX to registry') {
            steps {
                dir ("${WORKSPACE}/streaming_server/") {
                    sh '''
                         docker push 709233559969.dkr.ecr.eu-west-3.amazonaws.com/computer_vision/computer_vision_nginx:${CV_VER}
                       '''
               }
             }
        }
