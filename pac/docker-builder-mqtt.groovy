    ...
    stages {
            ...
            steps {
                dir ("${WORKSPACE}/mqtt_broker/") {
                    sh '''
                         ...
                         docker build . -t 709233559969.dkr.ecr.eu-west-3.amazonaws.com/computer_vision/computer_vision_mqtt:${CV_VER}
                       '''
                }
             }
          }
        stage ('push computer vision MQTT to registry') {
            steps {
                dir ("${WORKSPACE}/mqtt_broker/") {
                    sh '''
                         docker push 709233559969.dkr.ecr.eu-west-3.amazonaws.com/computer_vision/computer_vision_mqtt:${CV_VER}
                       '''
               }
             }
        }
