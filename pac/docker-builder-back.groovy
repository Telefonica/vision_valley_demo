    ...
    stages {
            ...
            steps {
                dir ("${WORKSPACE}/server/") {
                    sh '''
                         ...
                         docker build . -t 709233559969.dkr.ecr.eu-west-3.amazonaws.com/computer_vision/computer_vision_back:${CV_VER}
                       '''
                }
             }
          }
        stage ('push computer vision backend to registry') {
            steps {
                dir ("${WORKSPACE}/server/") {
                    sh '''
                         docker push 709233559969.dkr.ecr.eu-west-3.amazonaws.com/computer_vision/computer_vision_back:${CV_VER}
                       '''
               }
             }
        }
